...
const ARCHETYPES: Archetype[] = [
  ...
  },
  {
    ...
    formation: equalize([
      /*
       * КОМПАКТНО, в три лейна, а не россыпью по пяти.
       *
       * Замер против «стены со стрелками»: россыпью 3%, компактно 95%.
       * Те же шесть слотов кавалерии, разница только в форме.
       *
       * Причина — лейновая изоляция. Растянутая по пяти лейнам конница ведёт
       * пять отдельных маленьких боёв и в каждом оказывается в меньшинстве.
       * Собранная в три — имеет локальный перевес, а при квадратичном законе
       * локальный перевес превращается в разгром.
       *
       * Это тот же закон Ланчестера, который мы весь проект укрощали, только
       * теперь он работает НА игрока: концентрация бьёт распыление. Правило
       * старое как военное дело, и хорошо, что оно выводится из нашей модели
       * само, а не прописано вручную.
       */
      slot(0, 0, UnitClass.Cavalry), slot(1, 0, UnitClass.Cavalry),
      slot(2, 0, UnitClass.Cavalry), slot(0, 1, UnitClass.Cavalry),
      slot(1, 1, UnitClass.Cavalry), slot(2, 1, UnitClass.Cavalry),
    ]),
    isProbe: false,
  },
  {
  ...
  },
  {
    ...
    formation: equalize([
      /*
       * НАЛЁТЧИКИ СТОЯТ ВПЕРЕДИ, и это не интуитивно, зато измерено.
       *
       * Против «фронта с магами» — того самого состава, у которого не было
       * управы:
       *   4 налётчика ЗА двумя павизами  →  20%
       *   4 налётчика ВПЕРЕДИ + 2 лучника →  98%
       *
       * Причина в скорости. Павиза самая медленная в игре, налётчик самый
       * быстрый. Поставленные за стеной, налётчики упираются в собственный
       * фронт, теряют разгон и вязнут в чужой пехоте вместо того, чтобы
       * дойти до магов. Их единственное преимущество — прорыв — тратится
       * впустую.
       *
       * Два лучника сзади нужны не для урона, а как страховка от ЧУЖОГО
       * налёта: лучник единственный контрит налётчика (+120%), и состав без
       * него сам становится лёгкой добычей.
       */
      slot(0, 0, UnitClass.Skyrider), slot(1, 0, UnitClass.Skyrider),
      slot(2, 0, UnitClass.Skyrider), slot(3, 0, UnitClass.Skyrider),
      slot(1, 2, UnitClass.Archer), slot(2, 2, UnitClass.Archer),
    ]),
    isProbe: false,
  },
...
function main(): void {
  ...
  /*
   * --- ЕСТЬ ЛИ У КАЖДОГО СОСТАВА УПРАВА ---
   *
   * Эта метрика важнее «доли непредрешённых», и вот почему.
   *
   * Жёсткие контры ПО ОПРЕДЕЛЕНИЮ делают матчап предрешённым: если лучник
   * обязан выносить налётчика, то клетка обязана быть близка к 100. Гоняться
   * за долей неопределённых исходов, имея жёсткие контры, — значит требовать,
   * чтобы контры не работали.
   *
   * Настоящая болезнь меты другая: состав, которому нечего противопоставить.
   * Если на каждый состав есть хотя бы один, кто его уверенно бьёт, мета
   * живая — даже когда отдельные пары решаются всухую. Если у кого-то управы
   * нет, он становится обязательным, и выбора у игрока не остаётся.
   *
   * Считаем только по смешанным составам: моно-зонды и бьют, и получают
   * неестественно.
   */
  console.log('\nУПРАВА НА КАЖДЫЙ СОСТАВ (кто уверенно бьёт, ≥60%):\n')
  let noAnswer = 0
  let mixCount = 0
  for (let j = 0; j < m; j++) {
    if (ARCHETYPES[j]!.isProbe) continue
    mixCount++
    let best = -1
    let bestWr = 0
    for (let i = 0; i < m; i++) {
      if (i === j || ARCHETYPES[i]!.isProbe) continue
      if (wr[i]![j]! > bestWr) {
        bestWr = wr[i]![j]!
        best = i
      }
    ...
    if (bestWr >= 0.6) {
      console.log(
        `  ${ARCHETYPES[j]!.name.padEnd(24)} ← ${ARCHETYPES[best]!.name} ` +
          `(${(bestWr * 100).toFixed(0)}%)`,
      )
    } else {
      noAnswer++
      console.log(
        `  ${ARCHETYPES[j]!.name.padEnd(24)} ← ⛔ УПРАВЫ НЕТ, лучший ответ ` +
          `${best >= 0 ? ARCHETYPES[best]!.name : '—'} всего ${(bestWr * 100).toFixed(0)}%`,
      )
    }
  }
  console.log(
    `\n  Составов без управы: ${noAnswer} из ${mixCount}  ← цель 0`,
  )
